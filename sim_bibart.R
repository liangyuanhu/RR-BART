...
#path<-"/Volumes/Backup Plus/Mount Sinai/LH PCORI/SWAN/102721/"
...
impute_boot<-NA
impute_boot<-foreach(i=1:n_rep, .combine="comb", .multicombine = TRUE,  .init=list(list(), list(), list()), 
                     .packages = c("doParallel")) %:%
  
  foreach(b=1:n_boot_impute, .combine="comb", .multicombine = TRUE, .init=list(list(), list(), list()),
          .packages = c("doParallel")) %dopar%{
            ...
            list(selected_var_b, vs_prop_factor, vs_permutate_factor)
            
          }

stopCluster(cl)
saveRDS(impute_boot,  paste(path,"x",seedx,"impute_boot_missrfbart_102721.rds",sep=""))
#impute_boot<-readRDS("C:/winscpsync/n4out/finalMatrix_fullbart_102721.rds")
#stop cluster
Sys.time()


# Select final local vars and Calculate metrics  (precision, recall, f1, AUC)
final_var0<-list(NA)
...
for(i in 1:n_rep) {
  ...
  metrics_mat_local<-data.frame(matrix(NA, nrow=length(cutpt_boot_varselect), ncol=5))
  names(metrics_mat_local)<-c("Cutpt","Precision","Recall","F1","AUC")
  ...
  for(j in 1:length(cutpt_boot_varselect)){
    ...
    if(length(final_var_i$local_finalvar)>0){
      ...
      prec_recall_f1_met_local[[j]]<-list("tp"=NA, "fp"=NA, "fn"=NA, 
                                              "precision"=NA, "recall"=NA, "f1"=NA)
    }
    
    ## Precision, recall f1
    metrics_mat_local[j,c(1:4)]<-c(cutpt_boot_varselect[j], prec_recall_f1_met_local[[j]]$precision, 
                             prec_recall_f1_met_local[[j]]$recall, prec_recall_f1_met_local[[j]]$f1)
    
    ## AUC
    cov_mat<-mice_valid_dat[,final_local_var_i[[j]], drop = F]
    out_vec<-factor(as.character(mice_valid_dat$y), levels=c("Yes","No"))
    bm_valid<-bartMachine::bartMachine(y = out_vec, X=cov_mat, mem_cache_for_speed=FALSE,
    ...
    ### AUC
    metrics_mat_local[j,5]<-auc(roc(response=mice_valid_dat$y, predictor=mice_valid_dat$phat, 
...
for(i in 1:length(cutpt_boot_varselect)){
  ...
  names(metrics_mat_local_list_i)<-c("Cutpt","Precision","Recall","F1","AUC")
...
metrics_mat_list<-list("met_local"=metrics_mat_local_list, "final_var"=final_var0, 
                       "prec_recall_f1_met"=prec_recall_f1_met0)
# Export
saveRDS(metrics_mat_list, paste(path,"x",seedx, "metrics_mat_missrfBART_031021_na06.rds",sep=""))